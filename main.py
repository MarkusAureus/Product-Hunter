...
def search(event=None):
    ...
    # Vymaze kazde predchodze najdene z Listboxu (output)
    output.delete(0, END)        
    # URL stránky, kterou chceš prohledat
    ...
    if response.status_code == 200:
        # Načtení HTML obsahu pomocí BeautifulSoup
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # Identifikace cílových elementů (například pro název a cenu produktu)
        product_list = soup.find_all('div', class_='inzeraty')
        
        # Procházení výsledků a extrakce informací
        for product in product_list:
            ...
            if name_element and price_element:
                ...
                output.insert(END,f"{name}, Cena: {price}")
                output.insert(END, '\n') 
